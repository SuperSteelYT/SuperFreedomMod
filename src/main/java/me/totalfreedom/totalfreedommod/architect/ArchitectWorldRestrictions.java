package me.totalfreedom.totalfreedommod.architect;
...
public class ArchitectWorldRestrictions extends FreedomService
{
    ...
    public ArchitectWorldRestrictions(TotalFreedomMod plugin)
    {
        super(plugin);
    }
    ...
    {
        if (!plugin.arl.isArchitect(player) && !FUtil.isExecutive(player.getName()) && player.getWorld().equals(plugin.wm.architectWorld.getWorld()))
        {
            return true;
        }
    ...
    public void onCommandPreprocess(PlayerCommandPreprocessEvent event)
    {
        ...
        if (doRestrict(player))
        {
            ...
            {
                player.sendMessage(ChatColor.RED + "Only Architects are allowed to use this WorldEdit command in the Architect world.");
                event.setCancelled(true);
            }

            if (plugin.al.isAdmin(player) && !plugin.al.isSeniorAdmin(player) && message.startsWith("/co"))
            {
                player.sendMessage(ChatColor.RED + "Only Architects are allowed to use CoreProtect in the Architect world.");
                event.setCancelled(true);
            }
        }
    }
