...
@CommandParameters(description = "Manage my admin entry", usage = "/<command> [-o <admin>] <clearips | clearip <ip> | setlogin <message> | clearlogin | settag <tag> | cleartag | setacformat <format> | clearacformat> | logstick>")
    ...
    protected boolean run(CommandSender sender, Player playerSender, Command cmd, String commandLabel, String[] args, boolean senderIsConsole)
    {
        ...
        switch (args[0])
        {
            ...
            case  "setacformat":
            {
                ...
                String example = format.replace("%name%", "ExampleAdmin").replace("%rank%", Rank.SENIOR.getAbbr()).replace("%rankcolor%", Rank.SENIOR.getColor().toString()).replace("%msg%", "The quick brown fox jumps over the lazy dog.");
