...
import me.totalfreedom.totalfreedommod.rank.Rank.Type;
...
public class RankManager extends FreedomService
{
    ...
    public Displayable getDisplay(CommandSender sender)
    {
        ...
        if (ConfigEntry.SERVER_DONATORS.getList().contains(player.getName()) && !plugin.al.isAdmin(player))
        {
            return Title.DONATOR;
        }

        // Architects show up if they are not admins
        if (plugin.arl.isArchitect(player) && !plugin.al.isAdmin(player))
        {
            return Title.ARCHITECT;
        }

        // TotalFreedom Developers always show up
        if (FUtil.TFDEVELOPERS.contains(player.getName()))
        {
            return Title.TFDEVELOPER;
        }
    ...
    {
        if (plugin.al.isAdminImpostor(player) || plugin.pv.isPlayerImpostor(player) || plugin.arl.isArchitectImpostor(player))
        {
            return Rank.IMPOSTOR;
        }
    ...
    public void updateDisplay(Player player)
    {
        ...
        if (plugin.al.isAdmin(player))
        {
            ...
            fPlayer.setTag(display.getPublicTag());
    ...
    public void onPlayerJoin(PlayerJoinEvent event)
    {
        ...
        // Handle impostors
        Boolean isImposter = plugin.al.isAdminImpostor(player) || plugin.pv.isPlayerImpostor(player) || plugin.arl.isArchitectImpostor(player);
        if (isImposter)
        {
        ...
        // Set display
        if (isAdmin || FUtil.TFDEVELOPERS.contains(player.getName()) || ConfigEntry.SERVER_DEVELOPERS.getList().contains(player.getName()) || plugin.arl.isArchitect(player) || ConfigEntry.SERVER_DONATORS.getList().contains(player.getName()))
        {
            ...
            plugin.pl.getPlayer(player).setTag(display.getPublicTag());
        ...
        if (ConfigEntry.OP_ON_JOIN.getBoolean())
        {
            player.setOp(true);
        }
    }
